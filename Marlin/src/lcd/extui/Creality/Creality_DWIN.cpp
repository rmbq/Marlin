  ...
  void onIdle()
  {
    ...
    if (awaitingUserConfirm() && (lastPauseMsgState != ExtUI::pauseModeStatus || userConfValidation > 99))
    {
      ...
      case PAUSE_MESSAGE_PARKING:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_PAUSE_PRINT_PARKING));
        break;
      case PAUSE_MESSAGE_CHANGING:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_INIT));
        break;
      case PAUSE_MESSAGE_UNLOAD:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_UNLOAD));
        break;
      case PAUSE_MESSAGE_WAITING:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_ADVANCED_PAUSE_WAITING));
        break;
      case PAUSE_MESSAGE_INSERT:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_INSERT));
        break;
      case PAUSE_MESSAGE_LOAD:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_LOAD));
        break;
      case PAUSE_MESSAGE_PURGE:
#if ENABLED(ADVANCED_PAUSE_CONTINUOUS_PURGE)
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_CONT_PURGE));
        break;
#else
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_PURGE));
        break;
#endif
      ...
      case PAUSE_MESSAGE_HEAT:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEAT));
        break;
      case PAUSE_MESSAGE_HEATING:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEATING));
        break;
      case PAUSE_MESSAGE_OPTION:
        ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_FILAMENT_CHANGE_OPTION_HEADER));
        break;
      case PAUSE_MESSAGE_STATUS:
        SERIAL_ECHOLNPGM_P(PSTR("PauseStatus"));
        break;
      default:
        onUserConfirmRequired(PSTR("Confirm Continue"));
        break;
      }
  ...
  void RTSSHOW::RTS_HandleData()
  {
    ...
    case DWIN_VP_PID_LVL:
      if (recdat.data[0] == 0)
        ...
        ; // TODO: start nozzle calib
      else if (recdat.data[0] == 2)
        ; // TODO: start bed calib
      else if (recdat.data[0] == 3)
        storeSettings();
      break;
    case DWIN_VP_LEVELING:
      if (recdat.data[0] == 0)
        ; // TODO: torno a settings se non ancora cominciato o gia' finito
      else if (recdat.data[0] == 1)
        ; // TODO: home axis
      else if (recdat.data[0] == 2)
        ; // TODO: z +0.1mm
    ...
    case DWIN_VP_TEMP:
      ...
      else if (recdat.data[0] == 3)
      {
        ...
        if (FanStatus)
          SERIAL_ECHOLNPGM_P(PSTR("FAN IS NOW ON"));
        else
          SERIAL_ECHOLNPGM_P(PSTR("FAN IS NOW OFF"));
      ...
      {
        if (recdat.data[0] == 5)
          ; // TODO: impostare temperature PLA
        else if (recdat.data[0] == 6)
          ; // TODO: impostare temperature ABS
      ...
      else if (recdat.data[0] == DWIN_BUTTON_NO)
        ; // TODO: do nothing?
      break;
    case DWIN_VP_FILE_SEL:
      ...
      else if (recdat.data[0] == 1)
      {
        ...
        ; // TODO: scorro giu' (visualizzo elementi sucessivi)
      else if (recdat.data[0] == 3)
        ; // TODO: scorro su' (visualizzo elementi preccedenti)
      else if (recdat.data[0] == 4)
...
// InforShowStatus = true;
        ...
        if (ExtUI::getMeshValid())
        {
          ...
          for (uint8_t outer = 0; outer < GRID_MAX_POINTS_Y; outer++)
          {
            for (uint8_t inner = 0; inner < GRID_MAX_POINTS_X; inner++)
            {
              ...
              bool zig = (outer & 1);
              if (zig)
                x_Point = (GRID_MAX_POINTS_X - 1) - inner;
  ...
  void SetTouchScreenConfiguration()
  {
    ...
    if (Settings.display_sound)
      cfg_bits |= 1UL << 3; // 3: audio
    if (Settings.display_standby)
      cfg_bits |= 1UL << 2; // 2: backlight on standby
    if (Settings.screen_rotation == 10)
      cfg_bits |= 1UL << 1; // 1 & 0: Inversion
